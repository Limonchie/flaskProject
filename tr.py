...
def education_children_under14():
    if flask.request.method == 'GET':
        ...
        studentphone = flask.request.form.get('studentphone')
        studentemail = flask.request.form.get('studentemail')
        study_plan = "Практический курс китайского языка для детей (от 14 до 18 лет)"
        exam_selection = flask.request.form.get('examselection')
        exam_date = flask.request.form.get('examdate')
        submission_date = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            ...
            try:
                ...
                if table_name == 'students':
                    student_data = {
                        ...
                        'age_group': student[8],
                        'id_type': student[9],
                        'id_details': student[10],
                        'bank_details': student[11],
                        'phone': student[12],
                        'email': student[13],
                        'study_plan': student[14],
                        'exam_selection': student[15],
                        'exam_date': student[16],
                        'status': student[17],
                        'submission_date': student[18],
                        'representative': {
